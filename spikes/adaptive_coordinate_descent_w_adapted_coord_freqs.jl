...
# selected directions?
function acf_adaptive_coordinate_descent(
        fitnessfct, P, Xmin, Xmax;
        MaxEval = 1.0e4 * P,
        stopfitness = 1.0e-8,
    ...
    bestFit = 1.0e+100 # Start from a very high fitness to ensure we will find lower... Fix smarter...
        ...
        if currentpos >= 2 * P
            ...
            if (numfevals - lastconvergencecheck) >= (MaxEval * nonImprovementBudget / 100.0)
                ...
                if pfitness > 1000 * stopfitness
                    ...
                    xm2, bf2, nf2 = acf_adaptive_coordinate_descent(
                        fitnessfct, P, Xmin, Xmax;
...
function predicted_final_fitness(currentfevals, lastfevals, maxfevals, currentfitness, lastfitness)
    ...
    return currentfitness + slope * (maxfevals - currentfevals)
end
...
function update!(ae::AdaptiveEncoding, population::Matrix{Float64}, howOftenUpdateRotation = 1)
    ...
    return if ae.iter % howOftenUpdateRotation == 0 || ae.iter <= 2
