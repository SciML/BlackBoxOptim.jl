...
function my_fun1(x)
    return exp(-(x - 2) .^ 2) + 0.8 * exp(-(x + 2) .^ 2)
end

# Cross-entropy using a gaussian proposal distr...
function cross_entropy_method(
        func;
        max_iterations = 100, num_samples = 100, top_quantile = 0.1,
        std_convergence = 1.0e-5, mu = 0, sigma = sqrt(100), maximization = true
    )
