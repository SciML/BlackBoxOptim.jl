...
function directions_for_k(cg::ConstantDirectionGen, k)
    return cg.directions # Always the same regardless of k...
end
...
function generating_set_search(
        problem;
        max_seconds = 2 * numdims(problem), max_evals_per_dim = 1.0e5,
        ftol = 1.0e-7,
        delta_tol = 1.0e-13,
    ...
    step_size = 0.5 * minimum(diameters(ss))
...
function maximize_min_distance(
        n, points, num_samples = 100,
        sampler = () -> randn(n, 1)
    )
    xbest = xnew = sampler()
...
function restart_gss(
    ...
    while (now < start_time + max_seconds)
        ...
        x, fbest, num_fevals, termination_reason = generating_set_search(
            f, n;
...
function run_with_params(of, n)
    ...
    return println(join([Libc.strftime("%Y%m%d-%H%M%S", start_time), elapsed, of, n, fb, fes, tr, xb], ","))
end
