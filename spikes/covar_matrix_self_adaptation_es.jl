...
function linear_utilities(mu, lambda)
    return normalize_utilities(hcat(ones(1, mu), zeros(1, lambda - mu)))
end

function log_utilities(mu, lambda)
    return normalize_utilities(hcat((log(mu + 1) - log(1:mu))', zeros(1, lambda - mu)))
end
...
function cmsa_es(
        ...
        max_seconds = 4 * numdims(p), max_evals_per_dim = 1.0e7,
        ftol = 1.0e-7, xtol = 1.0e-10, stol = 1.0e-10,
...
function restart_cmsa_es(
        p;
        trace = true,

        # Stopping criteria related
        max_seconds = 2 * numdims(p), max_evals_per_dim = 1.0e7,
        ftol = 1.0e-7, xtol = 1.0e-10, stol = 1.0e-10,
    ...
    while (now < start_time + max_seconds)
        ...
        x, fbest, num_fevals, termination_reason, archive = cmsa_es(
            p;
        ...
        sigma = 0.1 * rand(1:4) * minimum(diameters(search_space(p)))
...
function update_covariance_matrix!(cms::CovarianceMatrixSampler, delta, a)
    C = a * cms.C + (1 - a) * delta
    return cms.C = triu(C) + triu(C, 1)' # Ensure C is symmetric. Should not be needed, investigate...
end
...
function multivariate_normal_sample(cms::CovarianceMatrixSampler, n, m)
    return cms.B * (cms.diagD .* randn(n, m))
end
