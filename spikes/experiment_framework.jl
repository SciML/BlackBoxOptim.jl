function sumstats(v, f = (x) -> @sprintf("%.2f", x))
    ...
    return "median = $(f(median(v))), mean = $(f(mean(v))) +/- $(f(std(v))), range = [$(f(minimum(v))), $(f(maximum(v)))]"
end

function format_time(t)
    return if t < 5.0e-1
        @sprintf("%.2f ms", t * 1.0e3)
    elseif t < 60.0
        @sprintf("%.2f s", t)
    elseif t < 30 * 60
...
function unique_filename(prefix = "result", suffix = ".txt")
    return join([prefix, Libc.strftime("_%Y%m%d_%H%M%S_$(rand(1:int(1.0e6)))", time()), suffix])
end

function csvfile(
        header; fileprefix = "experiment",
    ...
    for i in 0:num_runs
        ...
        for pi in problem_indices
            ...
            if add_summary_csv_header
                summary_csvfh = csvfile(
                    [
                        "Experiment", "Date", "Time", "RunId",
                        "Problem", "Dimension", paramheader,
                        "ElapsedTime", "FuncEvals", "TerminationReason",
                        "Fitness",
                    ]; filepath = join([file_prefix, "_summary.csv"])
                )
            ...
            println(
                summary_csvfh, join(
                    [
                        experiment, Libc.strftime("%Y-%m-%d", start_time),
...
function run_test(f, n; mu = false)
    of = xtransform(n, f)
    tic()
    x, f, fevals, reason = cmsa_es(
        n, of; covarMatrixSampler = EigenCovarSampler,
        mu = mu,
    ...
    return println("fevals/dim = $(fevals / n)")
end
