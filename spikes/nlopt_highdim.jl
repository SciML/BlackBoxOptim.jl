...
GlobalNLoptAlgs = [
    :GN_DIRECT, :GN_DIRECT_L, :GN_ORIG_DIRECT, :GN_ORIG_DIRECT_L,
...
function compare_nlopt_algs(
        ...
        Algs = [
            :LN_COBYLA, :LN_BOBYQA, :LN_NEWUOA, :LN_PRAXIS, :LN_NELDERMEAD, :LN_SBPLX,
    ...
    for maxt in MaxTimes
        for alg in Algs
            for rep in 1:NumReps
                ...
                res = rbind(
                    res, DataFrame(
                        Alg = alg, D = dim, Rep = rep, Fevals = numfevals,
...
function combine_nlopt_algs(
        func, dim;
        Algs = nothing, RelTimes = nothing,
        LowBound = -100.0,
        HighBound = 100.0,
        MaxTime = nothing,
        ResetToBestProbability = 0.8,
        FTOL_REL = -Inf,
        FTOL_ABS = -Inf
    )
    ...
    if Algs == nothing
        ...
        Algs = vcat(
            [:GN_DIRECT_L, :LN_PRAXIS], random_pairs_of_algs(numpairs),
            shuffle(LocalNLoptAlgs), [:LN_PRAXIS]
        )
        RelTimes = vcat([1, 3], rand(1:3, 2 * numpairs), 0.3 * rand(length(LocalNLoptAlgs)), 2)
    end
    ...
    for i in 1:numsteps
        ...
        if (nowtime - timebest) > 0.2 * MaxTime
...
SOPS = [
    sphere, ellipsoid, elliptic, cigar, cigtab, ackley, schwefel1_2,
...
function xrotatedandshifted(n, f, shiftAmplitude = 1.0, rotateAmplitude = 1.0)
    ...
    return transformed_f(x) = f(rotmatrix * (x .- shift))
end
...
function compare_optimizers(
        optimizers, problems, dim, numreps = 10;
