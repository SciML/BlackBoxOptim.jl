...
function step(ep::EvolutionaryProgrammingStrategy)
    ...
    for i in 1:pep.population_size
        children[:, i], childsigmas[:, i] = mutate_individual(
            ep, currentpop, currentsigmas,
            i, pep.dimension, pep.tauprim, pep.tau
        )
    end

    # Select the individuals to survive and copy them to the next population.
    return select_winning_individuals(ep, currentpop, currentsigmas, children, childsigmas, nextpop, nextsigmas)

end

function select_winning_individuals(
        ep::EvolutionaryProgrammingStrategy,
...
function sample_tauprimconst_distribution(ep::EvolutionaryProgrammingStrategy)
    return randn() # Standard is to use a N(0, 1) gaussian...
end
...
function sample_mutation_distribution(ep::EvolutionaryProgrammingStrategy, numsamples::Int)
    return randn(numsamples) # Standard is to use a N(0, 1) gaussians...
end
