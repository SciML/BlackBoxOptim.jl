function test_epsilon_dominance_func(epsilon_dominates_func)
    describe("with scalar epsilon") do
        ...
        test("simple cases with float vector fitness values of length 2") do
            @check epsilon_dominates_func([1.0, 2.0], [1.0, 2.0], 0.1)
            @check epsilon_dominates_func([1.1, 2.0], [1.0, 2.0], 0.1)
            ...
            @check !epsilon_dominates_func([1.1, 2.1], [1.0, 2.0], 0.1)
            @check !epsilon_dominates_func([2.1, 1.1], [2.0, 1.0], 0.1)
            @check !epsilon_dominates_func([0.1, 1.1], [0.0, 1.0], 0.1)
            @check !epsilon_dominates_func([-0.9, 0.1], [-1.0, 0.0], 0.1)
            @check !epsilon_dominates_func([2.0, 3.0], [1.0, 2.0], 0.5)
        end
    ...
    return describe("with vector epsilon") do
...
describe("epsilon_dominance") do
    describe("epsilon_dominance_clear") do
        test_epsilon_dominance_func(epsilon_dominates_clear)
    end

    describe("epsilon_dominance_fast") do
