...
function rosenbrock(x)
    ...
    return sum(i -> 100 * abs2(x[i + 1] - x[i]^2) + abs2(x[i] - 1), Base.OneTo(length(x) - 1))
end
...
bboptimize(
    optfun; SearchRange = (-5.0, 5.0), NumDimensions = 2,
...
@time res_single = bboptimize(
    optfun; SearchRange = (-5.0, 5.0), NumDimensions = 100,
