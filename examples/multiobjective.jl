...
plot(
    layer(
        x = [x.orig[1] for x in values(pareto_pts)], # Draw the exact Pareto frontier
        y = [x.orig[2] for x in values(pareto_pts)],
        Geom.line
    ),
    layer(
        x = [fitness(x).orig[1] for x in pareto_frontier(res)], # the result of the method
