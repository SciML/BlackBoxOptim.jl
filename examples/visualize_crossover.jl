...
and given `parents`.
"""
function plot_crossover(
        xover::CrossoverOperator, parents::Matrix{Float64};
        n = 5000, shuffle_parents = false
    )
    ...
    plot(
        view(children_mtx, 1, :), view(children_mtx, 2, :),
    ...
    return scatter!(
        view(parents_pop.individuals, 1, :), view(parents_pop.individuals, 2, :),
        markersize = 5, markercolor = colorant"purple", label = "parents"
    )
end

plot_crossover(
    SimplexCrossover{3}(1.2),
    [[0.5, 0.0] [-0.2, 0.0] [0.0, 1.0]]
)

plot_crossover(
    SimulatedBinaryCrossover(0.2, 5.0),
    [[1.0, 0.2] [-0.2, 1.0]]
)

plot_crossover(
    UnimodalNormalDistributionCrossover{2}(0.5, 0.05),
    [[0.5, 0.0] [1.0, 3.0]]
)

plot_crossover(
    UnimodalNormalDistributionCrossover{3}(0.5, 0.05),
    [[0.5, 0.0] [1.0, 3.0] [3.0, 1.0]]
)

plot_crossover(
    ParentCentricCrossover{3}(0.1, 0.2),
    [[0.5, 0.0] [1.0, 3.0] [3.0, 1.0]]
)
