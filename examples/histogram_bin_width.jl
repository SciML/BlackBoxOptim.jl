...
function find_optimal_bin_width(data; minwidth = nothing, maxwidth = nothing)
    ...
    best, fitness = bboptimize(
        (delta) -> c_objective(delta[1], sorted), (min_width, max_width);
        dimensions = 1, iterations = 1.0e4
    )

    return best[1], count_events_per_bin(best[1], sorted)
end
