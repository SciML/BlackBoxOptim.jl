...
HB = 1.0e6 # This should be Inf but let's be practical...
...
@assert isapprox(best_fitness(res), 0.54433105, atol = 1.0e-6)
best = best_candidate(res)
@assert isapprox(best[1], 0.3333333, atol = 1.0e-6)
@assert isapprox(best[2], 0.2962963, atol = 1.0e-6)
