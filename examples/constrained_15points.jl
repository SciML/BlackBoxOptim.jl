...
prob = BlackBoxOptim.minimization_problem(
    points15, "Constrained points 15",
    (0.0, 1.0), 45, -0.64
)
...
function penalty(x, range)
    lower = x .< range[1]
    higher = x .> range[2]
    return 100 * (norm(range[1] - x[lower]) + norm(x[higher] - range[2]))
end
...
# Run XNES
result = bboptimize(
    penalized_points15; SearchRange = (0.0, 1.0), NumDimensions = Dims,
...
result = bboptimize(
    points15; SearchRange = (0.0, 1.0), NumDimensions = Dims,
    MaxTime = MaxMinutes * 60
)
