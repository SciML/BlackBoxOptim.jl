...
mutable struct FunctionBasedProblem{F, FS <: FitnessScheme, SS <: SearchSpace, FO} <: OptimizationProblem{FS}
    ...
    function FunctionBasedProblem(
            objfunc, name::String,
            fitness_scheme::FS, search_space::SS,
            opt_value::FO = nothing
        ) where {FS <: FitnessScheme, SS <: SearchSpace, FO}
        if FO <: Number
        ...
        return new{typeof(objfunc), FS, SS, typeof(opt_value)}(objfunc, name, fitness_scheme, search_space, opt_value)
    end
end
...
Base.copy(p::FunctionBasedProblem) =
    ...
    deepcopy(p.objfunc), deepcopy(p.name),
