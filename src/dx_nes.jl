...
mutable struct DXNESOpt{F, E <: EmbeddingOperator} <: ExponentialNaturalEvolutionStrategyOpt
    ...
    function DXNESOpt{F}(
        ...
        return new{F, E}(
            embed, lambda, u, Vector{Float64}(undef, lambda),
...
const DXNES_DefaultOptions = chain(
...
function dxnes(problem::OptimizationProblem, parameters)
    ...
    return DXNESOpt{fitness_type(problem)}(
...
function ask(dxnes::DXNESOpt)
    ...
    return update_candidates!(dxnes, dxnes.Z)
end
...
function assign_distance_weights!(
