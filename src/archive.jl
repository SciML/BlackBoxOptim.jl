...
mutable struct TopListArchive{F <: Number, FS <: FitnessScheme} <: Archive{F, FS}
    ...
    function TopListArchive(
            fit_scheme::FS, numdims::Integer,
            capacity::Integer = 10
        ) where {FS <: FitnessScheme}
        F = fitness_type(FS)
        return new{F, FS}(
            fit_scheme, time(), numdims, 0, capacity,
...
function delta_fitness(a::TopListArchive)
    return if length(a.fitness_history) < 2
        Inf
    else
...
function add_candidate!(
        a::TopListArchive{F}, fitness::F, candidate::AbstractIndividual,
        tag::Int = 0, num_fevals::Int = -1
    ) where {F}
    a.num_fitnesses += 1
    if (num_fevals == -1)
        num_fevals = a.num_fitnesses
    end
    ...
    return if length(a) < capacity(a) ||
        ...
        fs = fitness_scheme(a)
        ix = searchsortedfirst(
            a.candidates, new_cand,
...
function save_fitness_history_to_csv_file(
    ...
    for af in a.fitness_history
        ...
        line = [
            line_prefix, Libc.strftime("%Y-%m-%d,%T", af.timestamp),
