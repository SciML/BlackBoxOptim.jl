...
mutable struct DiffEvoOpt{P <: Population, S <: IndividualsSelector, M <: GeneticOperator, E <: EmbeddingOperator} <: PopulationOptimizer
    ...
    function DiffEvoOpt(
        ...
        return new{P, S, M, E}(name, pop, select, modify, embed)
    end
end
...
function evolve(de::DiffEvoOpt, crossover::CrossoverOperator)
    ...
    trial = acquire_candi(de.population, target)
    apply!(
        crossover, trial.params, target_index,
...
"""
function evolved_pair(
        de::DiffEvoOpt, target::Candidate{F}, trial::Candidate{F},
...
function adjust!(de::DiffEvoOpt, candi::Candidate, is_improved::Bool)
    ...
    return adjust!(
        candi.extra::GeneticOperator, candi.tag, candi.index, candi.fitness,
...
function diffevo(
        problem::OptimizationProblem, options::Parameters, name::String,
