...
struct ParetoFitnessScheme{N, F <: Number, MIN, AGG} <: TupleFitnessScheme{N, F, NTuple{N, F}, MIN, AGG}
    ...
    ParetoFitnessScheme{N}(;
        fitness_type::Type{F} = Float64,
...
struct EpsDominanceFitnessScheme{N, F <: Number, MIN, AGG} <: FitnessScheme{NTuple{N, F}}
    ...
    function EpsDominanceFitnessScheme{N, F}(
        ...
        return new{N, F, is_minimizing, AGG}(ϵ, aggregator)
    end
end

EpsDominanceFitnessScheme{N}(
...
struct EpsBoxDominanceFitnessScheme{N, F <: Number, MIN, AGG} <: TupleFitnessScheme{N, F, IndexedTupleFitness{N, F}, MIN, AGG}
    ...
    EpsBoxDominanceFitnessScheme{N, F}(
        ϵ::Union{F, Vector{F}};
        is_minimizing::Bool = true, aggregator::AGG = sum
    ) where {N, F <: Number, AGG} =
        new{N, F, is_minimizing, AGG}(check_epsbox_ϵ(ϵ, N), aggregator)

    EpsBoxDominanceFitnessScheme{N}(
        ϵ::Union{F, Vector{F}};
        is_minimizing::Bool = true, aggregator::AGG = sum
    ) where {N, F <: Number, AGG} =
        new{N, F, is_minimizing, AGG}(check_epsbox_ϵ(ϵ, N), aggregator)
...
Base.convert(
    ::Type{IndexedTupleFitness{N, F}}, fitness::NTuple{N, F},
    fs::EpsBoxDominanceFitnessScheme{N, F}
) where {N, F} =
    IndexedTupleFitness(fitness, aggregate(fitness, fs), fs.ϵ, Val{is_minimizing(fs)})

Base.convert(
    ::Type{IndexedTupleFitness}, fitness::NTuple{N, F},
    fs::EpsBoxDominanceFitnessScheme{N, F}
) where {N, F} =
    IndexedTupleFitness(fitness, aggregate(fitness, fs), fs.ϵ, Val{is_minimizing(fs)})

Base.convert(
    ::Type{NTuple{N, F}}, fitness::IndexedTupleFitness{N, F},
    fs::EpsBoxDominanceFitnessScheme{N, F}
) where {N, F} = fitness.orig

hat_compare(
    u::IndexedTupleFitness{N, F}, v::IndexedTupleFitness{N, F},
    fs::EpsBoxDominanceFitnessScheme{N, F}, expected::Int = 0
) where {N, F} =
    hat_compare_ϵ_box(u, v, is_minimizing(fs), expected)

hat_compare(
    u::NTuple{N, F}, v::IndexedTupleFitness{N, F},
    fs::EpsBoxDominanceFitnessScheme{N, F}, expected::Int = 0
) where {N, F} =
    hat_compare(convert(IndexedTupleFitness{N, F}, u, fs), v, fs, expected)

hat_compare(
    u::IndexedTupleFitness{N, F}, v::NTuple{N, F},
    fs::EpsBoxDominanceFitnessScheme{N, F}, expected::Int = 0
) where {N, F} =
...
hat_compare(
    ...
    hat_compare(
    convert(IndexedTupleFitness{N, F}, u, fs),
...
(hc::HatCompare{FS})(
    u::IndexedTupleFitness{N, F},
    v::IndexedTupleFitness{N, F}
) where {FS <: EpsBoxDominanceFitnessScheme, N, F} =
