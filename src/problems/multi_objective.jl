...
struct Hypersurface{N, SS <: SearchSpace}
    ...
    function Hypersurface(manifold::Function, parameter_space::SS) where {SS <: SearchSpace}
        ...
        return new{N, SS}(manifold, parameter_space)
    end
end
...
function nondominated(fitnesses, fit_scheme::EpsBoxDominanceFitnessScheme{N, F}) where {N, F}
    ...
    return IndexedTupleFitness{N, F}[fitness(pt) for pt in pareto_frontier(arch)]
end
...
"""
function IGD(
        ref::Hypersurface{N}, sol::AbstractVector{T},
...
function CEC09_UP8_PF(t::Vector{Float64}, ::Type{Val{NP}}) where {NP}
    d = sum(abs2, t)
    return (t[1], t[2], d <= 1.0 ? (1.0 - d)^(1 / 3) : NaN)
end
...
const CEC09_Unconstrained_Set = Dict{Int, FunctionBasedProblemFamily}(
    8 => FunctionBasedProblemFamily(
        CEC09_UP8, "CEC09 UP8", ParetoFitnessScheme{3}(is_minimizing = true),
        (-2.0, 2.0),
...
const Schaffer1Family = FunctionBasedProblemFamily(
