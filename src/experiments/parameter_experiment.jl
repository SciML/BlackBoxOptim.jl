...
function design01_matrix_from_R_list(fromR, matrix_name = "best")
    return reshape(fromR[matrix_name], fromR["num_rows"], fromR["num_cols"])
end

function map01_range_to_design_range(value01, design_min, design_max)
    return design_min + value01 * (design_max - design_min)
end
...
function summary_of_R_output(pe::ParameterExperiment, designfile = "new_runs.json")
    ...
    return println("Sensitivity analysis, decreasing total effects: ", pe.parameters[perm])
end
...
function run_with_parameters_from_json_file(
        pe::ParameterExperiment, designfile, searchfunc, problem;
    ...
    return run_based_on_design_matrix_in_file_while_saving_results_to_csvfile(
        searchfunc,
...
function explore_parameters(
    ...
    experiment_name = join(
        [
            experiment_prefix, name(problem), numdims(problem),
            nps, "params",
        ], "_"
    )
    ...
    run_based_on_design_matrix_in_file_while_saving_results_to_csvfile(
        searchfunc,
        problem, pe, outfile; num_repeats = num_repeats, designfile = designfile
    )
