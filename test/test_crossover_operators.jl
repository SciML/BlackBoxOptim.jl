@testset "Crossover operators" begin
    ...
    @testset "DiffEvoRandBin1" begin
        @testset "always copies from donor if length is 1" begin
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(0.0, 0.0),
                [0.0], 4, DE.population, [1, 2, 3]
            ) == [3.0]
        end
        ...
        @testset "unlikely to copy everything if vectors are large" begin
            for i in 1:NumTestRepetitions
                ...
                saved_target = copy(target)
                res = BlackBoxOptim.apply!(
                    BlackBoxOptim.DiffEvoRandBin1(0.1, 0.5),
                    target, 1, pop, [2, 3, 4]
                )
        ...
        @testset "correctly modifies the parameters vector" begin
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(1.0, 0.4),
                [0.0], 4, DE.population, [1, 2, 3]
            ) == [3.0 + (0.4 * (1.0 - 2.0))]
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(1.0, 0.4),
                [0.0], 4, DE.population, [2, 3, 1]
            ) == [1.0 + (0.4 * (2.0 - 3.0))]
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(1.0, 0.4),
                [0.0], 4, DE.population, [3, 2, 1]
            ) == [1.0 + (0.4 * (3.0 - 2.0))]
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(1.0, 0.4),
                [0.0], 4, DE.population, [1, 3, 5]
            ) == [5.0 + (0.4 * (1.0 - 3.0))]
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(1.0, 0.4),
                [0.0], 3, DE.population, [4, 9, 8]
            ) == [8.0 + (0.4 * (4.0 - 9.0))]

            pop2 = reshape(collect(1.0:8.0), 4, 2)'
            @test BlackBoxOptim.apply!(
                BlackBoxOptim.DiffEvoRandBin1(1.0, 0.6),
                [0.0, 0.0], 4, pop2, [1, 2, 3]
            ) == [
                3.0 + (0.6 * (1.0 - 2.0)),
                7.0 + (0.6 * (5.0 - 6.0)),
            ]
