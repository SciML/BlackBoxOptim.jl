...
@testset "Top-level interface" begin
    ...
    bboptimize(
        rosenbrock; SearchRange = (-5.0, 5.0), NumDimensions = 2,
    ...
    @testset "run a simple optimization" begin
        ...
        @testset "using non-population optimizer" begin
            res = bboptimize(
                rosenbrock; Method = :generating_set_search,
        ...
        @testset "using population optimizer" begin
            res = bboptimize(
                rosenbrock; Method = :adaptive_de_rand_1_bin,
                SearchRange = (-5.0, 5.0), NumDimensions = 2,
                MaxSteps = 5000, TraceMode = :silent
            )
            @test best_fitness(res) < 0.1
    ...
    @testset "continue running an optimization after it finished" begin
        optctrl = bbsetup(
            rosenbrock; SearchRange = (-5.0, 5.0), NumDimensions = 100,
            MaxTime = 0.5, TraceMode = :silent
        )
    ...
    @testset "return results after interruption" begin
        ...
        @testset ":RecoverResults on" begin
            i = 0
            optctrl = bbsetup(
                rosenbrock_throwing; SearchRange = (-5.0, 5.0), NumDimensions = 100,
    ...
    @testset "TargetFitness option works" begin
        ...
        result1 = bboptimize(
            rosenbrock, SearchRange = (-5.0, 5.0), NumDimensions = 5,
            Method = :de_rand_1_bin, FitnessTolerance = 1.0e-5,
        ...
        @test best_fitness(result1) < 1.0e-5
    ...
    @testset "callback" begin
        ...
        NumCalls = 30
        res = bboptimize(
            rosenbrock; SearchRange = (-5.0, 5.0), NumDimensions = 20,
            TraceMode = :silent,
    ...
    @testset "callable as objective" begin
        ...
        @test best_candidate(res) ≈ zeros(2) atol = 1.0e-5
    end
